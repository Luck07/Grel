...
/*!
 *  @brief  Implements missing powf2 function
 *  @param  x
 *          Base number
 *  @param  y
 *          Exponent
 ...
 */
float powf2(const float x, const float y) {
 ...
 *  @param  value
 */
void tcs_soft::write8(uint8_t reg, uint8_t value) {
 ...
 *  @return value
 */
uint8_t tcs_soft::read8(uint8_t reg) {
 ...
 *  @return value
 */
uint16_t tcs_soft::read16(uint8_t reg) {
 ...
 *          Gain
 */
tcs_soft::tcs_soft(uint8_t it, tcs34725Gain_t gain) {
 ...
 */
boolean tcs_soft::begin(SoftwareWire *theWire, uint8_t addr) {
  if (i2c_dev)
    delete i2c_dev;
  i2c_dev = new dafruit_I2CDevice(theWire, addr);
 ...
 *          Integration Time
 */
void tcs_soft::setIntegrationTime(uint8_t it) {
 ...
 */
void tcs_soft::setGain(tcs34725Gain_t gain) {
  if (!_tcs34725Initialised)
    begin();
 ...
 *          Clear channel value
 */
void tcs_soft::getRawData(uint16_t *r, uint16_t *g, uint16_t *b,
                                   uint16_t *c) {
  if (!_tcs34725Initialised)
    begin();
 ...
 *          Clear channel value
 */
void tcs_soft::getRawDataOneShot(uint16_t *r, uint16_t *g, uint16_t *b,
                                          uint16_t *c) {
  if (!_tcs34725Initialised)
    begin();
 ...
 */
void tcs_soft::getRGB(float *r, float *g, float *b) {
 ...
 */
uint16_t tcs_soft::calculateColorTemperature(uint16_t r, uint16_t g,
                                                      uint16_t b) {
  ...
  cct =
      (449.0F * powf2(n, 3)) + (3525.0F * powf2(n, 2)) + (6823.3F * n) + 5520.33F;
 ...
 *  @return Color temperature in degrees Kelvin
 */
uint16_t tcs_soft::calculateColorTemperature_dn40(uint16_t r,
                                                           uint16_t g,
                                                           uint16_t b,
                                                           uint16_t c) {
 ...
 *  @return Lux value
 */
uint16_t tcs_soft::calculateLux(uint16_t r, uint16_t g, uint16_t b) {
  float illuminance;
 ...
 */
void tcs_soft::setInterrupt(boolean i) {
 ...
 *          High limit
 */
void tcs_soft::setIntLimits(uint16_t low, uint16_t high) {
